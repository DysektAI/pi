...
The lower-level agent harness can generate compactions that embed retained post-compaction context directly on the entry, instead of `firstKeptEntryId`. This `retainedTail` form is not produced or consumed by coding-agent's `SessionManager`:
...
- `retainedTail`: Harness-only materialized `AgentMessage[]` kept after compaction. Coding-agent sessions use `firstKeptEntryId` instead.
...
Coding-agent's `buildContextEntries()` walks from the current leaf to the root and applies the latest compaction boundary:

1. Collects entries back through the compaction's `firstKeptEntryId`
2. Includes the compaction entry and entries appended after it
3. Preserves non-message entries in the selected range so interactive mode can render them

Coding-agent's `buildSessionContext()` then extracts model/thinking settings and converts selected entries to messages. A compaction contributes its `compactionSummary`; retained messages come from the entries selected by `firstKeptEntryId`. The agent harness has separate context-building APIs that understand `retainedTail` checkpoints.
