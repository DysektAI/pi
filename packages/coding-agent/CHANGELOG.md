...
- Added `pi.getExtensions()` to the extension API, returning a fresh snapshot of extensions loaded in the current session with canonical `name`, `path`, `scope` (`project`/`user`/`package`/`cli`), and `source` metadata. See [Extensions](docs/extensions.md#pigetextensions).
